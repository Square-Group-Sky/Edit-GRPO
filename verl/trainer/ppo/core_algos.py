...
class AdvantageEstimator(str, Enum):
    ...
    GRPO_GRADIENT_COS = "grpo_gradient_cos"
    GPG = "gpg"
...
@register_adv_est(AdvantageEstimator.GRPO_GRADIENT_COS)
def compute_grpo_gradient_cos_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    config: Optional[AlgoConfig] = None,
    is_q1: np.ndarray | None = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for Edit-GRPO with gradient cosine similarity.

    For each prompt group:
    - Sample q1: A_q1 = (r_q1 - mean_{non-q1}) / (std_{non-q1} + eps)
    - Other samples: A_i = (r_i - mean_{non-q1}) / (std_{non-q1} + eps)

    The mean and std are computed from non-q1 samples only (q1 excluded).
    Cosine term cos(g_i, g_q1) is added later in the actor.

    q1 identification (in priority order):
    1. If is_q1 is provided (bool array), use explicit markers
    2. Otherwise, the first sample in each uid group is treated as q1
    """
    scores = token_level_rewards.sum(dim=-1)
    bsz = scores.shape[0]
    ...
    with torch.no_grad():
        ...
        for gid in id2score:
            scores_list = id2score[gid]
            idx_list = id2idx[gid]
            n = len(scores_list)

            if n == 1:
                id2mean[gid] = torch.tensor(0.0, device=scores.device)
                id2std[gid] = torch.tensor(1.0, device=scores.device)
                continue

            if is_q1 is not None:
                q1_in_group = [i for i in idx_list if is_q1[i]]
            else:
                q1_in_group = [idx_list[0]]

            gen_scores_list = []
            for i, s in zip(idx_list, scores_list):
                if i not in q1_in_group:
                    gen_scores_list.append(s)

            if len(gen_scores_list) == 0:
                id2mean[gid] = torch.tensor(0.0, device=scores.device)
                id2std[gid] = torch.tensor(1.0, device=scores.device)
            elif len(gen_scores_list) == 1:
                gen_tensor = torch.stack(gen_scores_list)
                id2mean[gid] = torch.mean(gen_tensor)
                id2std[gid] = torch.tensor(1.0, device=scores.device)
            else:
                gen_tensor = torch.stack(gen_scores_list)
                id2mean[gid] = torch.mean(gen_tensor)
                id2std[gid] = torch.std(gen_tensor)
...
def compute_per_sample_gradient_cosine(
    logits: torch.Tensor,
    responses: torch.Tensor,
    group_indices: torch.Tensor,
    lambda_cos: float = 1.0,
    is_q1: torch.Tensor | None = None,
) -> torch.Tensor:
    """Compute per-sample gradient cosine similarity with q1 (Edit-GRPO).

    The gradient is approximated at the logits level:
        g_i = softmax(logits_i) - one_hot(response_i)

    For each group, computes cos(g_i, g_q1) for non-q1 samples.
    q1 samples always get cos_term = 0.

    q1 identification (in priority order):
    1. If is_q1 is provided (bool tensor), use explicit markers
    2. Otherwise, the first sample in each group is treated as q1
    """
    bsz, resp_len, vocab_size = logits.shape
    device = logits.device

    probs = torch.nn.functional.softmax(logits, dim=-1)
    one_hot = torch.nn.functional.one_hot(responses, num_classes=vocab_size).to(dtype=probs.dtype)
    g = probs - one_hot

    g_sq_norms = (g * g).sum(dim=(1, 2))
    g_norms = torch.sqrt(g_sq_norms + 1e-8)

    cos_term = torch.zeros(bsz, device=device, dtype=probs.dtype)

    unique_groups = group_indices.unique()
    for gid in unique_groups:
        mask = group_indices == gid
        indices_in_group = mask.nonzero(as_tuple=True)[0]
        n_in_group = len(indices_in_group)
        if n_in_group < 2:
            continue

        if is_q1 is not None:
            q1_mask = is_q1[indices_in_group]
            q1_indices = indices_in_group[q1_mask]
            non_q1_indices = indices_in_group[~q1_mask]
        else:
            q1_indices = indices_in_group[:1]
            non_q1_indices = indices_in_group[1:]

        if len(q1_indices) == 0 or len(non_q1_indices) == 0:
            continue

        q1_idx = q1_indices[0]
        g_q1 = g[q1_idx]
        norm_q1 = g_norms[q1_idx]

        for idx in non_q1_indices:
            dot_product = (g[idx] * g_q1).sum()
            cos_val = dot_product / (norm_q1 * g_norms[idx] + 1e-8)
            cos_term[idx] = lambda_cos * cos_val
